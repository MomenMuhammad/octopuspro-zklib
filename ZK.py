...
class ZkConnect:
    ...
    def __init__(self, host, port, endpoint, name, transmission=True):
        """
...
class ParseConfig:
    ...
    def _validate(cls, config):
        ...
        """
        if not all(key in config.keys() for key in ['endpoint']):
            raise Exception('endpoint key is missing!')
